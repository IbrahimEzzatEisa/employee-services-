...
function source_onchange() {
    ...
    if (document.getElementById("btnScan"))
        document.getElementById("btnScan").value = "مــــــســــح";

    if (document.getElementById("source"))
        DWObject.SelectSourceByIndex(document.getElementById("source").selectedIndex);

    SelectScanner(); // open latest selected scaner by m.salah 22-4-2015.
}

function SelectScanner() {
    var sel = document.getElementById("source");
    var val = $.cookie("scanner");
...
function acquireImage() {
    DWObject.SelectSourceByIndex(document.getElementById("source").selectedIndex);
    $.cookie("scanner", document.getElementById("source").options[document.getElementById("source").selectedIndex].innerHTML); // Save value into cookie by m.salah 22-4-2015.
    ...
    DWObject.PageSize = EnumDWT_CapSupportedSizes.TWSS_A3;
...
function loadSampleImage(nIndex) {
    ...
    switch (nIndex) {
        case 1:
            ImgArr = "../img/twain_associate1.png";
            break;
        case 2:
            ImgArr = "../img/twain_associate2.png";
            break;
        case 3:
            ImgArr = "../img/twain_associate3.png";
            break;
    }
...
function btnUpload_onclick() {
    ...
    if (DynamLib.detect.ssl == true)
        ...
        //DWObject.HTTPPort = _strPort;
        DWObject.HTTPPort = 64194;

    var CurrentPathName = unescape(location.pathname); // get current PathName in plain ASCII	
    var CurrentPath = CurrentPathName.substring(0, CurrentPathName.indexOf("/") + 1);
    debugger;
    //strActionPage = CurrentPath + "SaveToFile.php?id=" + _trxID.value; // the ActionPage's file path
    strActionPage = CurrentPath + "/api/Twain/getFileFromTw" ; // the ActionPage's file path
    if (_trxType.value != "")
        strActionPage = strActionPage + "&Add=" + _Add.value + "&type=" + _trxType.value;
    var redirectURLifOK = CurrentPath + "home.aspx?action=saved";
    ...
    var OnSuccess = function (httpResponse) {
        ...
        //    alert("تم حفظ الملف بنجاح.")//if save to file.
        //}
        window.open(redirectURLifOK, '_parent');
    };
